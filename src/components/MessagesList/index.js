...
const useStyles = makeStyles((theme) => ({
  ...
  // Define o tamanho do cotainer da menssagem
  textContentItem: {
    overflowWrap: "break-word",
    padding: "3px 100px 6px 6px",
  },
  // Define o tamanho do cotainer da menssagem
  ...
  //Ajuste dos status da mensagem e hora
  timestamp: {
  fontSize: 11,
  position: "absolute",
  bottom: 0,  // Ajuste para garantir que o timestamp não fique sobrepondo
  right: 5,   // Ajuste a posição à direita
  color: theme.mode === 'light' ? "#999" : "#d0d0d0",
  zIndex: 10, // Certifique-se de que o timestamp fique por cima ou por baixo do conteúdo, se necessário
  },
  //Ajuste dos status da mensagem e hora
...
const MessagesList = ({
  ...
  useEffect(() => {
    ...
    return () => {
      document.removeEventListener('keydown', handleCloseEmojiOptions);
    };
  }, [ticketId]);
  ...
  const checkMessageMedia = (message) => {
    const safeMediaUrl = normalizeMediaUrl(message.mediaUrl);
    ...
    if (message.mediaType === "image") {
      return safeMediaUrl ? <ModalImageCors imageUrl={safeMediaUrl} /> : null;
    }

    if (message.mediaType === "audio") {
      if (!safeMediaUrl) return null;
      return (
        <AudioModal url={safeMediaUrl} />
        // <audio controls>
      ...
      >
        ...
        <div className={classes.quotedMsg}>
          ...
          {message.quotedMsg.mediaType === "audio"
            && (
              <div className={classes.downloadMedia}>
                {normalizeMediaUrl(message.quotedMsg.mediaUrl) ? <AudioModal url={normalizeMediaUrl(message.quotedMsg.mediaUrl)} /> : null}
          ...
          {message.quotedMsg.mediaType === "video"
            && normalizeMediaUrl(message.quotedMsg.mediaUrl)
            && (
              <video
                className={classes.messageMedia}
                src={normalizeMediaUrl(message.quotedMsg.mediaUrl)}
                controls
              />
            )
          }
            ...
            && (
              <div className={classes.downloadMedia}>
                <Button
                  ...
                  variant="outlined"
                  target="_blank"
                  href={normalizeMediaUrl(message.quotedMsg.mediaUrl)}
                >
                  Download
                </Button>
              </div>
          ...
          {message.quotedMsg.mediaType === "image"
            && (
              normalizeMediaUrl(message.quotedMsg.mediaUrl) ? <ModalImageCors imageUrl={normalizeMediaUrl(message.quotedMsg.mediaUrl)} /> : null)
                ...
                <Stack direction={'row'} spacing={0.5} alignItems={'center'} >
                  ...
                  <div>
                    ...
                    <div className={clsx(classes.textContentItem, {
                      ...
                      {(message.mediaType === "image" && getBaseName(message.mediaUrl) === message.body) || (message.mediaType !== "audio" && message.mediaType != "reactionMessage" && message.mediaType != "locationMessage" && message.mediaType !== "contactMessage") && (
              ...
              >
                ...
                >
                  ...
                  {/* <span className={classes.timestamp}>   {format(parseISO(message.createdAt), "HH:mm")}  {renderMessageAck(message)}  </span> */}
                          ...
                          {/*Corrige o erro de renderização de leitura de mensagem*/ }
                          <span className={classes.timestamp}>
                              {message.isEdited
                                  ? "Editada " + format(parseISO(message.createdAt), "HH:mm")
                                  : format(parseISO(message.createdAt), "HH:mm")}

                              {/* Acknowledgment renderizado ao lado do timestamp */}
